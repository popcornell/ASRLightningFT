...
class PaddedBatch:
    ...
    def __init__(
        ...
        special_padding_keys=None,  # {"tokens": partial(batch_pad_right, absolut=True, value=2)}
        padding_kwargs={},
        ...
        if special_padding_keys is None:
            special_padding_keys = {}
        for key in self.__keys:
            ...
            if (padded_keys is not None and key in padded_keys) or (
                ...
                if key in special_padding_keys.keys():
                    # to handle special situations.
                    padded = PaddedData(*special_padding_keys[key](values))
                else:
